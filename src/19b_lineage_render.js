...
// Deterministic ±1 picker for non-primary edges so each edge always routes its
// elbow to the same side across re-renders / pan / zoom.
function edgeSide(parentId, childId) {
  let h = 2166136261;
  const s = parentId + ':' + childId;
  ...
  for (const [id, n] of lineage.nodes) {
    ...
    for (let j = 0; j < ranked.length; j++) {
      ...
      if (j === 0) {
        ...
        // Non-primary parents (e.g. crossover partner) route via a right-angle
        // detour on a side track outside the child column, so a long vertical
        // run doesn't sit on top of the column of unrelated nodes between them.
        const side = edgeSide(pid, id);
        const trackX = cS.sx + side * (14 + 10 * (j - 1));
        lineageCtx.moveTo(pS.sx, pS.sy);
        lineageCtx.lineTo(trackX, pS.sy);
        lineageCtx.lineTo(trackX, cS.sy);
        lineageCtx.lineTo(cS.sx, cS.sy);
      }
      lineageCtx.stroke();
